...
function basalt_test_define_0004() {
    basalt_run_single_direct_test(4, 'PASS: List Users Commands (No Login)', 'Do not log in, and see what commands are returned.', 'user_tests');
}
...
function basalt_test_define_0008() {
    basalt_run_single_direct_test(8, 'PASS: List Users With Logins (Manager Login)', 'Log in with a manager login, and see what users are returned.', 'user_tests', 'asp', '', 'CoreysGoryStory');
}
...
function basalt_test_0012($in_login = NULL, $in_hashed_password = NULL, $in_password = NULL) {
    ...
    $result = call_REST_API('GET', 'http://localhost/basalt/test/basalt_runner.php/json/users/logins/5', NULL, $api_result, $result_code);
    if (isset($result_code) && $result_code && (200 != $result_code)) {
        echo('<h3 style="color:red">RESULT CODE: '.htmlspecialchars(print_r($result_code, true)).'</h3>');
    } else {
        echo('<pre style="color:green">'.prettify_json($result).'</pre>');
