...
basalt_run_tests(151, 'ADVANCED XML PLACES TESTS PART 2', 'Verify POST, DELETE and Special Functions.');

// -------------------------- DEFINITIONS AND TESTS -----------------------------------

function basalt_test_define_0151() {
    basalt_run_single_direct_test(151, 'PASS: Create A New Location', 'Log in, and create a new simple location with no data at all.', 'dc_area_tests', 'MDAdmin', '', 'CoreysGoryStory');
}
...
function basalt_test_define_0152() {
    basalt_run_single_direct_test(152, 'PASS: Create A New Location At the Lincoln Memorial', 'Log in, and create a new location, loaded up as the Lincoln Memorial.', 'dc_area_tests', 'DCAdmin', '', 'CoreysGoryStory');
}
...
function basalt_test_define_0153() {
    basalt_run_single_direct_test(153, 'PASS: Delete One Single Place.', 'Log in, and delete one single location.', 'dc_area_tests', 'MDAdmin', '', 'CoreysGoryStory');
}
...
function basalt_test_define_0154() {
    basalt_run_single_direct_test(154, 'PASS: Delete Three Places.', 'Log in, and delete three individual locations.', 'dc_area_tests', 'MDAdmin', '', 'CoreysGoryStory');
}
...
function basalt_test_define_0155() {
    basalt_run_single_direct_test(155, 'PASS: Delete Every Place We Can See.', 'Log in, and delete all the places that we can see.', 'dc_area_tests', 'MDAdmin', '', 'CoreysGoryStory');
}
...
function basalt_test_define_0156() {
    basalt_run_single_direct_test(156, 'PASS: Delete Some Places In A Radius Search.', 'Log in, and delete a few of the places returned in a radius search.', 'dc_area_tests', 'VAAdmin', '', 'CoreysGoryStory');
}
...
function basalt_test_define_0157() {
    basalt_run_single_direct_test(157, 'PASS: Test Pagination.', 'Do not log in, and do a big search, looking for various pages.', 'dc_area_tests');
}
...
function basalt_test_0157($in_login = NULL, $in_hashed_password = NULL, $in_password = NULL) {
    ...
    if (isset($result_code) && $result_code && (200 != $result_code)) {
        ...
        echo('<div  style="text-align:left;display:table"><div id="test_0157_results_1_div" class="inner_closed">');
            echo('<h3 class="inner_header"><a href="javascript:toggle_inner_state(\'test_0157_results_1_div\')" style="font-weight:bold">The Whole Enchilada ('.count($places).'):</a></h3>');
            echo('<div class="main_div inner_container">');
                echo('<pre style="color:green">'.prettify_xml($result).'</pre>');
            echo('</div>');
        echo('</div></div>');
    ...
    do {
        ...
        if (isset($result_code) && $result_code && (200 != $result_code)) {
            ...
            if (!isset($places) || !is_array($places) || !count($places)) {
                ...
                echo('<div  style="text-align:left;display:table"><div id="test_0157_results_2_'.$current_page.'div" class="inner_closed">');
                    echo('<h3 class="inner_header"><a href="javascript:toggle_inner_state(\'test_0157_results_2_'.$current_page.'div\')" style="font-weight:bold">Page '.($current_page + 1).' ('.count($places).'):</a></h3>');
                    echo('<div class="main_div inner_container">');
                        echo('<pre style="color:green">'.prettify_xml($result).'</pre>');
                    echo('</div>');
                echo('</div></div>');
    ...
    do {
        ...
        if (isset($result_code) && $result_code && (200 != $result_code)) {
            ...
            if (!isset($places) || !is_array($places) || !count($places)) {
                ...
                echo('<div  style="text-align:left;display:table"><div id="test_0157_results_3_'.$current_page.'div" class="inner_closed">');
                    echo('<h3 class="inner_header"><a href="javascript:toggle_inner_state(\'test_0157_results_3_'.$current_page.'div\')" style="font-weight:bold">Page '.($current_page + 1).' ('.count($places).'):</a></h3>');
                    echo('<div class="main_div inner_container">');
                        echo('<pre style="color:green">'.prettify_xml($result).'</pre>');
                    echo('</div>');
                echo('</div></div>');
...
function basalt_test_define_0158() {
    basalt_run_single_direct_test(158, 'PASS: Test Count', 'Don\'t log in, and get back just a count.', 'dc_area_tests');
}
...
function basalt_test_define_0159() {
    basalt_run_single_direct_test(159, 'PASS: Get Just The IDs', 'We don\'t log in, and do the same searches as above.', 'dc_area_tests');
}
    ...
    do {
        ...
        if (isset($result_code) && $result_code && (200 != $result_code)) {
            ...
            if (!isset($places) || !is_array($places) || !count($places)) {
                ...
                echo('<div  style="text-align:left;display:table"><div id="test_0159_results_1_'.$current_page.'div" class="inner_closed">');
                    echo('<h3 class="inner_header"><a href="javascript:toggle_inner_state(\'test_0159_results_1_'.$current_page.'div\')" style="font-weight:bold">Page '.($current_page + 1).' ('.count($places).'):</a></h3>');
                    echo('<div class="main_div inner_container">');
                        echo('<pre style="color:green">'.prettify_xml($result).'</pre>');
                    echo('</div>');
                echo('</div></div>');
...
function basalt_test_define_0160() {
    basalt_run_single_direct_test(160, 'PASS: Do An Address Lookup Radius Search', 'We don\'t log in, and do a search, based on a string address (the White House).', 'dc_area_tests');
}
...
function basalt_test_define_0161() {
    basalt_run_single_direct_test(161, 'PASS: Do An Address Venue String Search', 'We log in, and do a search, based on a string search for an explicit venue name, then we try with wildcards.', 'dc_area_tests', 'MDAdmin', '', 'CoreysGoryStory');
}
...
function basalt_test_define_0162() {
    basalt_run_single_direct_test(162, 'PASS: Do A Street Address String Search', 'We don\'t log in, and do a search, based on a string search for a street address (1496 S. Main Street); then, use a couple of wildcards (%Main St% and 149%), and see what happens.', 'dc_area_tests');
}
...
function basalt_test_0162($in_login = NULL, $in_hashed_password = NULL, $in_password = NULL) {
    ...
    if (isset($result_code) && $result_code && (200 != $result_code)) {
        ...
        echo('<div  style="text-align:left;display:table"><div id="test_0162_results_3_div" class="inner_closed">');
            echo('<h3 class="inner_header"><a href="javascript:toggle_inner_state(\'test_0162_results_3_div\')" style="font-weight:bold">See the Results:</a></h3>');
            echo('<div class="main_div inner_container">');
            echo('<pre style="color:green">'.prettify_xml($result).'</pre>');
            echo('</div>');
        echo('</div></div>');
...
function basalt_test_define_0163() {
    basalt_run_single_direct_test(163, 'PASS: Do A Town String Search', 'We don\'t log in, and do a search, based on a string search for a town (Rockville); then, use a wildcard (Ro%), and see what happens.', 'dc_area_tests');
}
...
function basalt_test_define_0164() {
    basalt_run_single_direct_test(164, 'PASS: Do A State String Search', 'We don\'t log in, and do a search, based on a string search for a state (DE); then, use a wildcard (D%), and see what happens.', 'dc_area_tests');
}
...
function basalt_test_define_0165() {
    basalt_run_single_direct_test(165, 'PASS: Do A County String Search', 'We don\'t log in, and do a search, based on a string search for a county (Montgomery); then, use a wildcard (Montgomery%), and see what happens.', 'dc_area_tests');
}
...
function basalt_test_define_0166() {
    basalt_run_single_direct_test(166, 'PASS: Do A Town and State String Search', 'We don\'t log in, and do a search, based on a string search for a town and a state (Wilmington, DE); then, use a wildcard (W%, D%), and see what happens.', 'dc_area_tests');
}
